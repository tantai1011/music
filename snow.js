...
var snowStorm = (function(window, document) {
  ...
  var result = new Date().getHours();
  var nowday = new Date().getDate();
  var nowmonth = new Date().getMonth();
 
  if(nowday==7 || (nowday==14 && nowmonth==1))
  {
      this.snowColor = 'red';        // Don't eat (or use?) yellow snow.
      this.snowCharacter = '❤'; 
  }
  else {
      if(nowday==31 && nowmonth==9){
        this.snowCharacter = '🎃'; 
      }
      else {
        if(nowday==25 && nowmonth==11){
            this.snowCharacter = '🎄'; 
        }    
        else {
              if(result>=18 || result==0 || result<6){
    	
    		    this.snowColor = '#efefef';        // Don't eat (or use?) yellow snow.
                this.snowCharacter = '❄'; 
              }
              else {
    		    this.snowColor = '#DE3163';        // Don't eat (or use?) yellow snow.
                this.snowCharacter = '❀'; 
              }
         }
      }  
